...
namespace Lusid.Sdk.Model
{
    ...
    public partial class ResourceListOfScopeDefinition :  IEquatable<ResourceListOfScopeDefinition>
    {
        ...
        public ResourceListOfScopeDefinition(List<ScopeDefinition> values = default(List<ScopeDefinition>), string href = default(string), int? count = default(int?), List<Link> links = default(List<Link>))
        {
            // to ensure "values" is required (not null)
            if (values == null)
            {
                throw new InvalidDataException("values is a required property for ResourceListOfScopeDefinition and cannot be null");
            }
            else
            {
                this.Values = values;
