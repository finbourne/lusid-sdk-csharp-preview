        ...
        public void ApiException_Converts_To_ValidationProblemDetails_AllowedRegex()
        {
            ...
            catch (ApiException e)
            {
                ...
                if (e.TryGetValidationProblemDetails(out var errorResponse))
                {
                    ...
                    Assert.That(errorResponse.Errors["code"].Single(), Is.EqualTo("Values for the field code must be comprised of either alphanumeric characters, hyphens or underscores (matching the regular expression '^[a-zA-Z0-9\\\\-_]+$'). For more information please consult the LUSID documentation."));
                    
                    //Should identify that there was a validation error with the scope
                    Assert.That(errorResponse.Errors, Contains.Key("scope"));
                    Assert.That(errorResponse.Errors["scope"].Single(), Is.EqualTo("Values for the field scope must be comprised of either alphanumeric characters, hyphens or underscores (matching the regular expression '^[a-zA-Z0-9\\\\-_]+$'). For more information please consult the LUSID documentation."));
